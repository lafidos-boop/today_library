  ...
  try {
    ...
    for (const sheetName of TARGET_SHEETS) {
      ...
      for (let i = dataStartIdx; i < rows.length; i++) {
        ...
        allBooks.push({
          id: code || `BD-${sheetName.substring(0,1)}-${i}`,
          title: title || '제목 없음',
          location: { shelf: String(row[colMap['서가']] || '-').trim(), room: sheetName },
          status: 'available'
        });
        count++;
      }
      console.log(`Sheet "${sheetName}": Mapped ${count} books.`);
    }
    await fs.writeFile(DB_PATH, JSON.stringify(allBooks, null, 2), 'utf8');
    console.log("Forced Sync Complete. Total:", allBooks.length);
  } catch (e) {
    console.error(e);
  }
}
