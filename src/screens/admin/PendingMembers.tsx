...
export const PendingMembers = ({
  ...
  <ScreenWrapper>
    <SubPageHeader icon={UserCheck} title="승인 대기 멤버" onBack={onBack} />

    <div className="bg-[#eeefe2] p-4 rounded-3xl mb-6">
      <p className="text-xs font-bold text-onSurfaceVariant opacity-70">
        승인 대기 중인 신청자 <span className="text-primary">{applicants.filter((a) => a.status === 'pending').length}</span>명
      </p>
    </div>
